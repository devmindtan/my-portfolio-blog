...
import { Search, X, ArrowUpDown, LayoutGrid, List, ChevronLeft, ChevronRight, ChevronDown, Check } from 'lucide-react';
import type { SortKey, SortDir, ViewMode } from '../data/portfolio';
    ...
    <div className="col-span-4 space-y-3">
      ...
      <div className="card-base flex items-center gap-2 px-3 py-2 flex-wrap sm:flex-nowrap">
        ...
        <div className="flex items-center gap-2 flex-1 min-w-0 group/search">
          <Search size={13} className="text-terminal-muted group-focus-within/search:text-terminal-accent transition-colors flex-shrink-0" />
          <input
            ref={searchRef}
            type="text"
            value={searchQuery}
            onChange={(e) => { onSearchChange(e.target.value); onPageChange(1); }}
            placeholder="grep -i [project, tech, tag...]"
          ...
          />
          {searchQuery ? (
            <button onClick={() => { onSearchChange(''); onPageChange(1); searchRef.current?.focus(); }} className="text-terminal-muted hover:text-terminal-accent transition-colors flex-shrink-0">
              <X size={13} />
            </button>
          ) : (
      ...
      <div className="flex flex-wrap gap-1.5">
        <button
          onClick={() => { onTagSelect(null); onPageChange(1); }}
        ...
        >
          all
          <span className="ml-1 text-terminal-muted/40 tabular-nums">{totalCount}</span>
        </button>
        {tags.map((tag) => (
          <button
            key={tag}
            onClick={() => { onTagSelect(selectedTag === tag ? null : tag); onPageChange(1); }}
            className={`px-2.5 py-1 text-[10px] uppercase tracking-wider border rounded-sm transition-all duration-200
              ${selectedTag === tag
                ? 'border-terminal-accent/40 text-terminal-accent bg-terminal-accent/5'
                : 'border-terminal-border/40 text-terminal-muted hover:border-terminal-borderHover hover:text-terminal-text'
          ...
          >
            {tag}
            <span className="ml-1 text-terminal-muted/40 tabular-nums">{tagCounts[tag] || 0}</span>
          </button>
        ))}
      </div>
      ...
      {isFiltered && (
        <div className="flex items-center gap-2 text-[10px] text-terminal-muted/60 flex-wrap">
          <span className="text-terminal-accent/50">active:</span>
          {selectedTag && (
            <span className="flex items-center gap-1 px-1.5 py-0.5 bg-terminal-accent/5 border border-terminal-accent/20 rounded-sm">
              tag={selectedTag}
              <button onClick={() => { onTagSelect(null); onPageChange(1); }} className="hover:text-terminal-accent transition-colors"><X size={9} /></button>
            </span>
          )}
          {searchQuery && (
            <span className="flex items-center gap-1 px-1.5 py-0.5 bg-terminal-info/5 border border-terminal-info/20 rounded-sm text-terminal-info/70">
              q="{searchQuery}"
              <button onClick={() => { onSearchChange(''); onPageChange(1); }} className="hover:text-terminal-info transition-colors"><X size={9} /></button>
            </span>
          )}
          <button
            onClick={() => { onTagSelect(null); onSearchChange(''); onPageChange(1); }}
        ...
        <div className="flex items-center justify-between text-[10px] text-terminal-muted/60">
          ...
          <div className="flex items-center gap-1">
            ...
            {getPageWindow(currentPage, totalPages).map((p, i) =>
              p === '...' ? (
                <span key={`ellipsis-${i}`} className="w-6 text-center text-terminal-muted/30">...</span>
              ) : (
                <button
                  key={p}
...
function SortDropdown({ sortKey, sortDir, onSortChange }: { sortKey: SortKey; sortDir: SortDir; onSortChange: (k: SortKey, d: SortDir) => void }) {
  ...
  return (
    <div ref={ref} className="relative flex-shrink-0">
      ...
      >
        ...
        <span className="text-terminal-accent/60">{sortDir === 'asc' ? 'ASC' : 'DESC'}</span>
      ...
      {open && (
        <div className="absolute top-full left-0 mt-1 w-40 bg-terminal-card border border-terminal-border rounded-sm shadow-xl shadow-terminal-bg/50 z-30 overflow-hidden opacity-0 animate-fade-in">
          {sortOptions.map((opt) => (
            <button
              key={opt.key}
              onClick={() => { onSortChange(opt.key, sortDir); setOpen(false); }}
              className="w-full flex items-center justify-between px-3 py-2 text-[10px] uppercase tracking-wider
                         text-terminal-muted hover:text-terminal-text hover:bg-terminal-highlight
                         transition-colors duration-150"
            >
              <span>{opt.label}</span>
              {sortKey === opt.key && <Check size={11} className="text-terminal-accent" />}
            </button>
          ))}
          <div className="border-t border-terminal-border/30" />
          <button
            onClick={() => { onSortChange(sortKey, sortDir === 'asc' ? 'desc' : 'asc'); setOpen(false); }}
            className="w-full flex items-center justify-between px-3 py-2 text-[10px] uppercase tracking-wider
                       text-terminal-muted hover:text-terminal-text hover:bg-terminal-highlight
                       transition-colors duration-150"
          >
